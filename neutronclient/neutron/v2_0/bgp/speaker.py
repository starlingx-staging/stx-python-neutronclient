...
def get_vpn_id(client, id_or_name):
    return neutronv20.find_resourceid_by_name_or_id(client,
                                                    'bgpvpn',
                                                    id_or_name)
...
class AddVpnToSpeaker(neutronv20.NeutronCommand):
    """Add a VPN to the BGP speaker."""

    def get_parser(self, prog_name):
        parser = super(AddVpnToSpeaker, self).get_parser(prog_name)
        parser.add_argument(
            'bgp_speaker',
            metavar='BGP_SPEAKER',
            help=_('ID or name of the BGP speaker.'))
        parser.add_argument(
            'vpn',
            metavar='BGPVPN',
            help=_('ID or name of the BGPVPN defined by bgpvpn-create.'))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        _speaker_id = get_bgp_speaker_id(neutron_client,
                                         parsed_args.bgp_speaker)
        _vpn_id = get_vpn_id(neutron_client,
                             parsed_args.vpn)
        neutron_client.add_vpn_to_bgp_speaker(_speaker_id,
                                              {'bgpvpn_id': _vpn_id})
        print(_('Added VPN %(bgpvpn)s to BGP speaker %(speaker)s.') %
              {'bgpvpn': parsed_args.vpn, 'speaker': parsed_args.bgp_speaker},
              file=self.app.stdout)


class RemoveVpnFromSpeaker(neutronv20.NeutronCommand):
    """Remove a VPN from the BGP speaker."""

    def get_parser(self, prog_name):
        parser = super(RemoveVpnFromSpeaker, self).get_parser(prog_name)
        parser.add_argument(
            'bgp_speaker',
            metavar='BGP_SPEAKER',
            help=_('ID or name of the BGP speaker.'))
        parser.add_argument(
            'vpn',
            metavar='BGPVPN',
            help=_('ID or name of the BGPVPN to remove.'))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        _vpn_id = get_vpn_id(neutron_client,
                             parsed_args.vpn)
        neutron_client.remove_vpn_from_bgp_speaker(_speaker_id,
                                                   {'bgpvpn_id': _vpn_id})
        print(_('Removed VPN %(bgpvpn)s from BGP speaker %(speaker)s.') %
              {'bgpvpn': parsed_args.vpn, 'speaker': parsed_args.bgp_speaker},
              file=self.app.stdout)
